 ...
 * JSCalendar
 * 
 * Author :H.Z. Shang (Jack) Email :shhongzhang@cntomorrow.com Site
 * :http://www.cntomorrow.com:3310 Version :1.0.1 Finished Date :2003-3-2
 * Beijing Huasun Mingtian Tech. Co., Ltd. No CopyRight,Can be modified by you
 * if you want improve it's function!!!! LET'S MAKE IT BETTER TOGETHER! HISTORY:
 * 1. [2003-3-4 by Jack] 采用IFRAME修正了会被页面SELECT对象挡住的BUG! 2. [2003-3-4 by Jack]
 * 增加了清空控件值的功能 3. [2003-3-5 by Jack] 修正了当用户翻年、翻月时原输入框值被覆盖的情况 4. [2003-3-5 by
 * Jack] 通过使用DXImageTransform.Microsoft.Shadow来实现IFRAME的阴影效果
 * 该滤镜只能在IE5.5下使用可加快程序运行速度，在IE5.0中还是使用原来的产生阴影的方法 5. [2003-3-6 by Jack]
 * 增加了另外一种生成该控件的方法：<script>createDateBox("Name", "2003-3-6")</script>
 * 其中第二个参数为可选参数
 */
...
/**
 * 在该日历控件中，我采用了给空间周围加上阴影层来增强控件的视觉感。 _nShadowLength 变量设定了阴影层的长度，默认值为 4；
 * 使用阴影层将相应的滞缓程序的运行速度，如果你想得到较快的运行速度，请将该值设为 0
 */
var _nShadowLength = 0;

/**
 * _sNeededFilePath 变量设定了JSCalendar控件所需的所有文件的位置 当你将你的应用部署到应用服务器上后，你需要给改变量设定相应的值。
 * 例如： _sNeededFilePath = "/MyWebAPP/comm/js/JSCalendar/";
 * 
...
function createCldTabFrm(){
	...
	// _cldTabIFrame2.style.pixelWidth = /*_innerTabFrmRect.right -
	// _innerTabFrmRect.left*/ //375 + _nShadowLength;
	// _cldTabIFrame2.style.pixelHeight = /*_innerTabFrmRect.bottom -
	// _innerTabFrmRect.top*/ 165 + _nShadowLength;
	...
	for(var d = 1, iRow = iDateStartRow; d <= _monthDays[_dCurMonth] || iRow < 9; ){
		for(; i < 7; i++){
			...
			if(dCurDate == _dCurDate && _td.name == "CURRENTMONTH"){
				// _cldTabFrm.rows(iRow).cells(i).style.backgroundColor =
				// "620662";
...
function placeCldTabFrm(){
	...
	_cldTabIFrame2.style.pixelWidth = /*
										 * _innerTabFrmRect.right -
										 * _innerTabFrmRect.left
										 */  220 + (_bBrowserVer55 ? 0 : _nShadowLength);
	_cldTabIFrame2.style.pixelHeight = /*
										 * _innerTabFrmRect.bottom -
										 * _innerTabFrmRect.top
										 */ 165+ (_bBrowserVer55 ? 0 : _nShadowLength);	
...
function switchLastMonth(bLast){
	...
	// _curCldTabRltvObj.value = _tmpdate.getFullYear() + "-" +
	// (_tmpdate.getMonth() + 1) + "-" + _tmpdate.getDate();
...
function switchLastYear(){
	// _curCldTabRltvObj.value = (_dCurYear * 1 + 1) + "-" + (_dCurMonth + 1) +
	// "-" + _dCurDate;
...
function switchNextYear(){
	// _curCldTabRltvObj.value = (_dCurYear - 1) + "-" + (_dCurMonth + 1) + "-"
	// + _dCurDate;
	setTargetFormaValue(_dCurYear * 1 - 1, _dCurMonth * 1 + 1, _dCurDate);
	_bHaveUpdated = false;
	// JSCalendar(_curCldTabRltvObj);
